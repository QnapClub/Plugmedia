        ...
        return this.each(function() {
            function addTag(tag) {
				...
				if (options.cssPrefix)
					item.attr('class', options.cssPrefix+'_'+Math.floor(Math.random()*7));
					
				if (options.deletable)
				{	
					item.attr('title', options.titleRemovalText);
					if (options.imageTag) {
						item.append('<img class="tag_delete" src="' + options.imageTagUrl + '">');
					}
					item.click(function() {
						...
						$.ajax({
							dataType: 'json',
							url: 'api.php?ac=remove_tag',
							data: 'values='+tag+'&file_id='+options.file_id,
							cache:false,
							async: false,
            ...
            function parse() {
			   	...
			   	if (textBase.val() != '')
				{
					...
					$.ajax({
						dataType: 'json',
						url: 'api.php?ac=create_tag',
						data: 'values='+textBase.val()+'&file_id='+options.file_id,
						cache:false,
						success: function(data) {
							$('#Tags').removeAttr('disabled');	
							if (data)
							{
								var items = data.split(options.separator);
