...
class CORE_exif {	
	...
	function formatData($type,$tag,$intel,$data) {
		//require_once ('upgrade_php/ext/gettext.php');
		if ($type == 'ASCII') {
			...
			if ($tag == '0112') { // Orientation
				...
				else if ($data == 3)    $data = ('Tungsten');	// 3 Tungsten (Incandescent light)
											// 4 Flash
											// 9 Fine Weather
				else if ($data == 10)   $data = ('Flash');	// 10 Cloudy Weather
											// 11 Shade
	...
	function formatExposure($data) {
		if ($data > 0) {
			if ($data > 1) {
				...
				$n=0; $d=0;
				$this->ConvertToFraction($data, $n, $d);
				return $n.'/'.$d.' '.('sec');
			}
		} else {
			return ('Bulb');
	...
	function read_entry(&$result,$in,$seek,$intel,$ifd_name,$globalOffset) {
		...
		if ($tag_name == 'MakerNote') { // if its a maker tag, we need to parse this specially
			...
			require_once('exif/makers/gps.php');
	...
	function read_exif_data_raw($path,$verbose) {
		...
		if (!$in || !$seek) {  // if the path was invalid, this error will catch it
			...
			log_message('error', "The file could not be found. (EXIF lib) : ".$path);
			return $result;
		}
	...
	if ($result['ValidJpeg'] == 1) {
		...
		if ($num<1000) { // 1000 entries is too much and is probably an error.
			...
			log_message('error', "Illegal size for IFD0. (EXIF lib) : ".$path);
		}
		...
		if ($v == -1) {
			...
			log_message('error', "Couldnt Find SubIFD. (EXIF lib) : ".$path);
		}
		...
		if ($num<1000) { // 1000 entries is too much and is probably an error.
			...
			log_message('error', "Illegal size for SubIFD. (EXIF lib) : ".$path);
		}
		
		// Add the 35mm equivalent focal length:
		$result['SubIFD']['FocalLength35mmEquiv'] = $this->get35mmEquivFocalLength($result);
		...
		if ($v == -1) {
			...
			log_message('error', "Couldnt Find IFD1. (EXIF lib) : ".$path);
		}
		...
		if ($num<1000) { // 1000 entries is too much and is probably an error.
			for($i=0; $i<$num; $i++) {
				$this->read_entry($result,$in,$seek,$intel,'IFD1',$globalOffset);
			}
		} else {
			$result['Errors'] = $result['Errors']+1;
			$result['Error'][$result['Errors']] = 'Illegal size for IFD1';
			log_message('error', "Illegal size for IFD1. (EXIF lib) : ".$path);
		...
		if ($v == -1) {
			...
			log_message('error', "Couldnt Find InteroperabilityIFD. (EXIF lib) : ".$path);
		}
		...
		if ($num<1000) { // 1000 entries is too much and is probably an error.
			for($i=0; $i<$num; $i++) {
				$this->read_entry($result,$in,$seek,$intel,'InteroperabilityIFD',$globalOffset);
			}
		} else {
			$result['Errors'] = $result['Errors']+1;
			$result['Error'][$result['Errors']] = 'Illegal size for InteroperabilityIFD';
			log_message('error', "Illegal size for InteroperabilityIFD. (EXIF lib) : ".$path);
		}
		fclose($in);
		fclose($seek);
		return $result;
	...
	function getShortExif($file)
	{
		$exif['IFD0'] = @exif_read_data($file, 'IFD0');
		return $exif;	
	}
