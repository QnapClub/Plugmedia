...
function smarty_modifier_bbcode2html($message) {
  $preg = array(
          ...
          '/(?<!\\\\)\[size(?::\w+)?=(.*?)\](.*?)\[\/size(?::\w+)?\]/si'     => "<span style=\"font-size:\\1em\">\\2</span>",
          ...
          // [code] & [php]
          '/(?<!\\\\)\[code(?::\w+)?\](.*?)\[\/code(?::\w+)?\]/si'           => "<div class=\"bb_code\">\\1</div>",
          '/(?<!\\\\)\[php(?::\w+)?\](.*?)\[\/php(?::\w+)?\]/si'             => "<div class=\"bb_php\">\\1</div>",
          // [email]
          ...
          // [quote]
          '/(?<!\\\\)\[quote(?::\w+)?\](.*?)\[\/quote(?::\w+)?\]/si'         => "<div class=\"bb-quotetop\">Citation</div><div class=\"bb-quote\">\\1</div>",
		  
         /* '/(?<!\\\\)\[quote(?::\w+)?=(?:&quot;|"|\')?(.*?)["\']?(?:&quot;|"|\')?\](.*?)\[\/quote\]/si'   => "<div class=\"bb-quotetop\">Quote \\1:</div><div class=\"bb-quote\">\\2</div>",*/
          ...
          '/\\\\(\[\/?\w+(?::\w+)*\])/'                                      => "\\1",
		  // [edit]
          '/(?<!\\\\)\[edit(?::\w+)?\](.*?)\[\/edit(?::\w+)?\]/si'           => "<br /><span style=\"font-size:0.7em\"><em>\\1</em></span>",		  
	  

  );
  //$message = preg_replace(array_keys($preg), array_values($preg), $message);
  include_once ('libs/class.bbcode2html.inc.php');
  $BBcode = new BBcode();
