...
function smarty_function_paginate_last($params, &$smarty) {
	...
	$last_page = ($_total % $_limit > 0) ? $_total - ( $_total % $_limit ) + 1 : $_total - $_limit + 1;
	
	if (SmartyPaginate::getCurrentItem($_id)!=$last_page)
	{
	
		$_url .= $last_page;
    
	    return '<a  href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . ' class="pagelink custom_target">' . $_text . '</a>';
    }
	else
		return '';
}
